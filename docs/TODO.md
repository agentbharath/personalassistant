...
- [ ] **Still rule-based (R20.5), to convert with the model and remove:** the clause split in `multi-agent.ts` (keyword lists that assign the clauses of a model-made plan to agents, plus a finance keyword check); `learning/commands.ts` parsers still used by handlers; `summarizeConversationTitle`. Each conversion needs eval cases first, then a small live re-run within R21.6 and R21.7.
...
- [x] **Email drafts built (R25.9, 2026-09-21), switched off.** Writer, recipient lookup, reply threading, preview and approval, edit, revert, discard, reconnect message, Settings row, Privacy Policy and Terms wording. Still to do by the owner: add `gmail.compose` to the Google consent screen, set `DRAFTS_ENABLED` and `NEXT_PUBLIC_DRAFTS_ENABLED` (local and Vercel), sign out and in once; lawyer review of the new legal text; try it with real mail. Not yet tested against a real Gmail account.
- [ ] Later, from feedback: consider offering Send, as its own decision (confirmation, delay, undo).
- [x] Draft reverting (R25.8): built with the rest of R25.9.
...
- [x] **Waiting on your reply** (R27, 2026-09-21): card on Perch for mail in Primary and Updates that seems to need a reply and has none in 7 days; a model judges (`reply-v1`, 29 cases verified live), remembered per message; dismissed for good (table `reply_dismissals`, migration 0018 applied). Opt-in (2026-09-21): the first visit asks what to be reminded about (people, companies, recruiters, invitations) and reads no mail until answered; Settings has Show Perch, reminders on/off and the kinds; table `perch_preferences` (migrations 0019, 0020). `reply-v2`, 36 cases verified live. Window is 7 days (owner decision 2026-09-21). Checks up to 30 new messages per visit, so a first visit costs about $0.07 at most.
- [ ] Waiting-on-reply follow-ups: a "Draft a reply" button once email drafts (R25) are on; mention it in the Privacy Policy (message text of Primary and Updates mail is sent to the model to judge it) before release; optionally include it in the chat `daily_view` answer.
- [x] Logging (R28, 2026-09-21): `reportFailure` and `logEvent`, swallowed failures now reported, background model calls counted against the user's budget and cost, `docs/logging.md`.
- [ ] **Monitoring and SLOs (next):** a scheduled job that runs `/api/ops/slo` plus a daily cost rollup and tells the owner; a deeper protected health check (database, cache, providers); provider error-rate and clarify / can't-interpret counters; extend SLO telemetry beyond chat (Perch, the reply scan); Sentry alert rules, uptime monitor, Vercel log retention and an Anthropic spend limit (owner actions).
- [x] "Near me" uses the saved home location (2026-09-21): router-v9 writes the web search (`searchQuery`) with the saved place in it, lets a named place win, and asks for a city when none is saved. Verified live on 28 cases; the one unverified case is "anything from my bank".
- [x] Search result cards and follow-ups (R29, 2026-09-21). Known gaps: cards show no photos, ratings or opening hours (only what the search evidence gives); no "directions" or drive time from the cards yet; the remembered list expires after an hour.
- [ ] Waiting-on-reply card: a "Draft a reply" button that starts the drafting flow (the Privacy Policy now says the text is sent to the model to judge it).
