...
- [ ] **Still rule-based (R20.5), to convert with the model and remove:** the email interpreter's message-based overrides and `email-turn`'s ordinal and correction patterns (R20.4); calendar date and place parsing (`getCalendarWindow`, `extractLocation`, `preciseStart`); the now-unused classifiers in `orchestrator/intent.ts`, `scope.ts`, `safety.ts` and `classifyWithClaude`; `learning/commands.ts` parsers still used by handlers; `summarizeConversationTitle`. Each conversion needs eval cases first, then a small live re-run within R21.6 and R21.7.
