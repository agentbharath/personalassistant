...
- [x] **Waiting on your reply** (R27, 2026-09-21): card on Perch for mail in Primary and Updates that seems to need a reply and has none in 14 days; a model judges (`reply-v1`, 29 cases verified live), remembered per message; dismissed for good (table `reply_dismissals`, migration 0018 applied). Opt-in (2026-09-21): the first visit asks what to be reminded about (people, companies, recruiters, invitations) and reads no mail until answered; Settings has Show Perch, reminders on/off and the kinds; table `perch_preferences` (migrations 0019, 0020). `reply-v2`, 36 cases verified live. Checks up to 20 new messages per visit, so a first visit costs about $0.05 at most.
