...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  // The error message can hold private data, so only its digest is logged; Sentry scrubs the event before it leaves.
  useEffect(() => { console.error("page_render_failed", error.digest ?? "no-digest"); Sentry.captureException(error); }, [error]);
