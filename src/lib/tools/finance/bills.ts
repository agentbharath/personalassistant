...
export type BillCandidate = { merchant: string; amountMinor: number; currency: string; category: string; statementDate: string; dueDate: string | null };

const COLUMNS = "id, merchant_ciphertext, amount_minor, currency, category, statement_date, due_date, status, paid_on";
const normalizeMerchant = (value: string) => value.toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, " ").trim();

type Row = { id: string; merchant_ciphertext: string; amount_minor: number | string; currency: string; category: string; statement_date: string; due_date: string | null; status: "outstanding" | "paid"; paid_on: string | null };
function decode(row: Row): Bill {
  return { id: row.id, merchant: decryptText(row.merchant_ciphertext), amountMinor: Number(row.amount_minor), currency: row.currency, category: row.category, statementDate: row.statement_date, dueDate: row.due_date, status: row.status, paidOn: row.paid_on };
}
...
export async function createBill(userId: string, candidate: BillCandidate, source: { externalRef?: string; payload?: string } = {}) {
  ...
  const fingerprint = piiHmac([candidate.statementDate, candidate.amountMinor, currency, normalizeMerchant(candidate.merchant)].join("|"));
...
export async function listBills(userId: string, status?: "outstanding" | "paid"): Promise<Bill[]> {
  ...
  let query = createAdminClient().from("finance_bills").select(COLUMNS).eq("user_id", userId).order("statement_date", { ascending: false }).limit(200);
