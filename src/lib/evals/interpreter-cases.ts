...
export type InterpreterCase = { label: string; input: InterpreterInput; expect: Partial<EmailRequest> & { domain?: "email" | "other" } };
...
export function checkInterpretation(result: Interpretation, want: InterpreterCase["expect"]) {
  ...
  for (const [key, value] of Object.entries(want)) {
    const got = key === "domain" ? result.domain : result.request[key as keyof EmailRequest];
