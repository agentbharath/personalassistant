...
export const ROUTER_VERSION = "router-v23";
...
- bills_list: what bills are outstanding or unpaid, all my dues, credit-card or utility amounts due, or what the user owes. This shows saved dues and checks email statements; new statements are previewed for confirmation before saving to Reminders. "Show my dues from saved bills and email" is bills_list. bills_paid: the user says they paid a bill: give merchant, and paidOn as an ISO date (YYYY-MM-DD) worked out from today when the user gives or implies a day ("yesterday", "last Sunday", "the 5th"), else null. bills_autopay: the user says a company's bill is on autopay (merchant).
