...
export const ROUTER_VERSION = "router-v29";
...
const EXAMPLES: Array<[string, ModelOutput]> = [
  ...
  ['"plan a trip to colorado this upcoming thanksgiving weekend"', { ...blank, operation: "multi", agents: ["calendar", "general"], searchQuery: "Thanksgiving activities in Colorado", confidence: 0.9, reading: "Check the dates are free, and search for what to do there" }],
...
- multi: one message with several separate asks across agents; list the agents involved in agents. When "general" is one of them, still fill searchQuery with a proper, complete search for just that part, exactly as web_search would ("plan a trip to Colorado this Thanksgiving weekend" -> agents [calendar, general], searchQuery "Thanksgiving activities in Colorado"): the raw sentence read out to a search engine is never as good as a query actually written for it.
...
export function canonicalizeDecision(raw: ModelOutput): Omit<RouterDecision, "source"> {
  ...
  switch (raw.operation) {
    ...
    case "multi": {
      ...
      return agents.length >= 2 ? { ...base, operation: "multi", agents, searchQuery: trim(raw.searchQuery ?? null, 300) } : ask("Which of those would you like first?");
    }
