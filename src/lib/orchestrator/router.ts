...
export const ROUTER_VERSION = "router-v13";
...
- approve / deny: the user answers a pending approval (pendingApproval is true) with yes, go ahead, looks good, or no, cancel, leave it, don't do that, in any wording. Read negatives carefully: "don't", "no", "not" mean deny. Only when pendingApproval is true. pendingApproval true means the system itself has an action waiting for the person's yes or no, whether or not recent shows it: trust the flag over what recent contains. When pendingApproval is true, any short agreement (yes, sure, ok, yep, sounds good, do it, please do) is approve even with no other context, and you must not ask what it refers to: the pending action is what it refers to. If the assistant's last message asked the user to say a confirmation phrase, a yes answers that request (for example "yes do it" after "Say yes, forget everything to confirm" is learning_forget with term everything).
...
export function buildRouterMessage(input: RouterInput) {
  ...
  return JSON.stringify({
    ...
    // Enough of the conversation to follow it: the last eight messages, with room for a list or a draft in Daylark's own answers, and the summary of
    // anything earlier when the conversation has one.
    summary: input.context.find((item) => item.content.startsWith("Earlier conversation summary"))?.content.replace(/\s+/g, " ").slice(0, 700) ?? null,
    recent: input.context.filter((item) => !item.content.startsWith("Earlier conversation summary")).slice(-8).map((item) => ({ role: item.role, text: item.content.replace(/\s+/g, " ").slice(0, item.role === "assistant" ? 700 : 300) })),
