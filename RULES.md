...
- **R19.8 Fallback.** Only when the router call fails (model error, no credit, budget used up) does the older rule chain run, and the source is logged as "rules". A router result and a rule result are never mixed for one message.
...
- **R20.3 Rules remain only as the fallback** when the model cannot be used (R19.8).
- **R20.4 Not yet converted (tracked):** the email interpreter (R16.4) still overrides the model with message-based checks (filters, windows, imports, "all", facts versus amounts, the exclusion clause), and the email turn still detects ordinal references and corrections with patterns. These are to be replaced by fields the model fills, and verified with the live eval.
